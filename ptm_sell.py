...
if __name__ == "__main__":
    ...
    IF_TR_accept = {
        ...
        "parameter" : {"orderId" : "id:order:0x123456"}
...
# 판매 정보 등록
    ...
    tmpOffer = client.recv()
    IF_TR_offer['parameter'].update(tmpOffer['parameter']) #tmpOffer['parameter']['offerId']
    #IF_TR_offer.update({"offerId" : tmpOffer1})  #IF_ID_onboard["parameter"] = 'true' 응답
    if tmpOffer['type'] == 'RES' :
        print("성공적으로 판매 정보가 등록되었습니다!")
        print("판매 정보의 offerId는 " + tmpOffer['parameter']['offerId'])
        print(IF_TR_offer)
...
# 구매 허가 (accept)
        ...
        IF_TR_accept['parameter'].update(tmpAccept['parameter'])
        if tmpAccept['type'] == 'RES' :
            print("구매요청을 수락하였습니다!")
            print("전력 거래 tradeId : " + tmpAccept['parameter']['tradeId'])
            print(IF_TR_accept)
...
# EMS에 거래결과 알림
            ...
            print('\n')
            IF_TR_contractMsg['parameter'] = IF_TR_accept['parameter']
            print(IF_TR_contractMsg)
            print('\n')
